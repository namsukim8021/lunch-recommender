...
## 추천 로직 (랜덤 + 최근 안 겹치게)
> **결정성(오라클 전제) — 설계 불변식**: 추천 난수원은 **주입 가능**해야 한다. 런타임은 `Math.random`, 검증(바이너리 오라클 golden regression)에는 **고정 시드 PRNG를 주입**한다(`Math.random`은 시드 불가). 예: `pickRandom(list, rng = Math.random)` 처럼 난수원을 파라미터화. 이게 있어야 [oracle.md](oracle.md) §1③ golden 스냅샷이 재현 가능하다.
